...
abstract contract MoonCashAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IMoonCashAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IMoonCashAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IMoonCashAuthority _authority) {
    ...
    function setAuthority(IMoonCashAuthority _newAuthority) external onlyGovernor {
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gCASH remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgCASH) external returns (uint256);
  function redeemAll(address _user, bool _sendgCASH) external returns (uint256);
...
interface IgCASH is IERC20 {
  ...
  function migrate( address _staking, address _sCASH ) external;
}

pragma solidity ^0.8.10;

abstract contract FrontEndRewarder is MoonCashAccessControlled {
  ...
  IERC20 internal immutable cash; // reward token

  constructor(
    IMoonCashAuthority _authority, 
    IERC20 _cash
  ) MoonCashAccessControlled(_authority) {
    cash = _cash;
  }
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in CASH (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in CASH
  }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  IgCASH internal immutable gCASH;
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _cash) {
    gCASH = _gcash;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of CASH due to the user
  ...
  function addNote(
    ...
    notes[_user].push(
      Note({
        payout: gCASH.balanceTo(_payout),
    ...
    // note that only the payout gets staked (front end rewards are in CASH)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgCASH    send payout as gCASH or sCASH
   * @return payout_     sum of payout sent, in gCASH
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgCASH) public override returns (uint256 payout_) {
    ...
    if (_sendgCASH) {
      gCASH.transfer(_user, payout_); // send payout as gCASH
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sCASH
    }
  }
   ...
   * @param _sendgCASH    send payout as gCASH or sCASH
   * @return             sum of payout sent, in gCASH
   */ 
  function redeemAll(address _user, bool _sendgCASH) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgCASH);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gCASH
...
/// @title MoonCash Bond Depository V2

contract MoonCashBondDepositoryV2 is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    ...
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _cash, _gcash, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _cash.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gCASH due
  ...
  function deposit(
     ...
     * price = quote tokens : cash (i.e. 42069 DAI : CASH)
     *
     * 1e18 = CASH decimals (9) + price decimals (9)
     */
     ...
     *
     * this is either the number of CASH that the market can sell
    ...
    // purchased, and how much CASH has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // cash decimals (9) + price decimals (9)
       ...
       * is 10,000 CASH. max payout would be 1,000 CASH (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in CASH or quote), initial price / CASH (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = cash decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 CASH, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 CASH.
     */
    ...
    emit CreateMarket(id_, address(cash), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in CASH decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of CASH to be paid in CASH decimals
   *
   * @dev 1e18 = cash decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in CASH decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in CASH decimals
   */
