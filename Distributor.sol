...
///         CASH, as well as patches a small bug in the staking contract
...
contract Distributor is MoonCashAccessControlled {
    ...
    /// The MoonCash Treasury
    ITreasury private immutable treasury;
    /// The CASH Staking Contract
    ...
    function triggerRebase() external {
        unlockRebase = true;
        IStaking(staking).unstake(msg.sender, 0, true, true); // Give the caller the bounty cash.
    ...
    function nextRewardFor(address who) public view returns (uint256) {
        return (cash.balanceOf(who) * rewardRate) / DENOMINATOR;
    }
