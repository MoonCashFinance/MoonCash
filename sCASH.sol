...
contract sMoonCash is IsCASH, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgCASH public gCASH; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked CASH", "sCASH", 9) ERC20Permit("Staked CASH") {
    ...
    function setgCASH(address _gCASH) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gCASH) == address(0), "gCASH:  gCASH already set");
        require(_gCASH != address(0), "gCASH:  gCASH is not a valid contract");
        gCASH = IgCASH(_gCASH);
    }
    ...
    /**
        @notice increases rCASH supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sCASH of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sCASH
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sCASH: insufficient balance");
    }
    ...
    // toG converts an sCASH balance to gCASH terms. gCASH is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gCASH.balanceTo(amount);
    }

    // fromG converts a gCASH balance to sCASH terms. sCASH is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gCASH.balanceFrom(amount);
    }

    // Staking contract holds excess sCASH
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gCASH.balanceFrom(IERC20(address(gCASH)).totalSupply())).add(
