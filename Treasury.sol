...
contract MoonCashTreasury is MoonCashAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for CASH
    ...
    function deposit(
        ...
        // mint CASH needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        CASH.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn CASH for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        CASH.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new CASH using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        CASH.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or CASH from the treasury, using sCASH as collateral. This might allow an
     * sCASH holder to provide CASH liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(CASH)) {
            require(permissions[STATUS.CASHDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sCASH.changeDebt(value, msg.sender, true);
        require(sCASH.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(CASH)) {
            CASH.mint(msg.sender, value);
            cashDebt = cashDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sCASH.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with CASH
     * @param _amount uint256
     */
    function repayDebtWithCASH(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.CASHDEBTOR][msg.sender], notApproved);
        CASH.burnFrom(msg.sender, _amount);
        sCASH.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        cashDebt = cashDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(CASH), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SCASH) {
            sCASH = IsCASH(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SCASH) {
            // 9
            sCASH = IsCASH(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(CASH.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns CASH valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(CASH)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return CASH.totalSupply() - cashDebt;
    }
}
